...
def get_data(path):  # get npy file
	...
	velocity = np.load('magnitude.npy') if os.path.exists('magnitude.npy') else get_velocity_data(path, vtu_num)
...
def LSTM_velocity(train, test, LSTM_epochs, sequence_length):
	...
	model.add(LSTM(1500, return_sequences=True, kernel_regularizer='l2', dropout=0.3,recurrent_dropout=0.2)) # add a layer with 50 neurons, type is lstm
	model.add(LSTM(2500, kernel_regularizer='l2', return_sequences=False, dropout=0.3,recurrent_dropout=0.2))
	...
	# model = EnKF.EnKF(12, 0.8, 32, LSTM_epochs, trainX, trainY, testX, testY, model)


	#ENKF part finish
	# fluidity_data = np.load('fluidity_data_ENKF.npy')
	# model = EnKF.ENKF_raw(fluidity_data.shape[1], 0.8, 512, LSTM_epochs, trainX, trainY, testX, testY, model,fluidity_data)
	# model = EnKF.EnKF(10, 0.8, 64, LSTM_epochs, trainX, trainY, testX, testY, model)

	model.fit(trainX, trainY, epochs=LSTM_epochs, batch_size=256, validation_data = (testX,testY),verbose = 1)
	# draw_acc_loss(history)
	model.save('magni_lstm(new).h5')
...
def train_velocity_model(velocity):
	...
	if not os.path.exists('magni_lstm(ENKF).h5'):
		encoder_model = 'D:/Master-Thesis/backward_facing_step_2d/vel_encoder(AE_10).h5'
...
if __name__=="__main__":  
	...
	print("Data loading...")
	velocity = np.load('magnitude.npy')
	print('Data Loaded!')
