...
def get_data(path):  # get npy file
	...
	velocity = np.load('MaterialVolumeFraction.npy') if os.path.exists('MaterialVolumeFraction.npy') else get_velocity_data(path, vtu_num)
...
def LSTM_velocity(train, test, LSTM_epochs, sequence_length):
	...
	# model = EnKF.ENKF_raw(fluidity_data.shape[1]//2, 0.8, 1, LSTM_epochs, trainX, trainY, testX, testY, model, fluidity_data)
	# ------------------#
	# model = EnKF.EnKF(10, 0.8, 1, LSTM_epochs, trainX, trainY, testX, testY, model)

	#ENKF part finish
	


	model.fit(trainX, trainY, epochs=LSTM_epochs, batch_size=32)
	# draw_acc_loss(history)
	model.save('Material_lstm(ENKF_full).h5')
...
def train_velocity_model(velocity):
	...
	if not os.path.exists('vel_lstm(ENKF_full).h5'):
		encoder_model = 'D:/Master-Thesis/water_collapse/vel_encoder(AE_10).h5'
...
if __name__=="__main__":  
	...
	print("Data loading...")
	velocity = np.load('MaterialVolumeFraction.npy')
