...
#EnKF part start
	...
	for i in range(len(indice) - 2):
		...
		if i < len(train_input) - 1:
			...
			observed_data = train_input[indice[i+1]+1, -1, :]
			...
			A = predicted_data - (predicted_data/ensemble_sample)
			C = (A.dot(np.transpose(A)))/ (ensemble_sample - 1)

			
			assimlated_data = predicted_data + C.dot(H).dot(inv(H.dot(C).dot(H) + R).dot((observed_data- H.dot(predicted_data))))
