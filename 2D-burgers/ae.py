...
from tensorflow.keras.layers import  LayerNormalization, Flatten
...
def ae_vol(vol, my_epochs, encoding_dim):# dim = 3
	...
	encoded = Dense(encoding_dim * 2, activation='softplus')(input_img)
	encoded = Dense(encoding_dim)(encoded)
	
	# "decoded" is the lossy reconstruction of the input
	decoded = Dense(encoding_dim * 2, activation='softplus',)(encoded)
	# decoded = Dense(encoding_dim * 4, activation='relu')(decoded)
	# decoded = Dense(encoding_dim * 16, activation='relu')(decoded)
	# decoded = Dense(encoding_dim * 32, activation='relu')(decoded)
	decoded = Dense(vol.shape[1], activation='softplus')(decoded)
	...
	history = autoencoder.fit(x_train, x_train, epochs=my_epochs, batch_size=16)
...
def train_vel(vol):
	...
	if not os.path.exists('vel_1encoder(AE_10).h5'):
