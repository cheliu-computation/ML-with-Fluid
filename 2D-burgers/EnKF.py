...
#EnKF part start
	...
	for i in range(len(indice) - 2):
		...
		if i < len(train_input) - 1:
			# print(i, train_input.shape, indice[i+1])
			observed_data = train_input[indice[i+1], -1, :ensemble_sample]
...
#EnKF part start
	...
	for i in range(len(indice) - 2):
		...
		if i < len(raw_data) - 1:
			print(indice[i+1])
			observed_data = raw_data[indice[i+1], -1, :ensemble_sample]
